const validatePurchaseOrder = (req, res, next) => {
    ...
    if (!supplierId || Number(supplierId) <= 0) {
        errors.push('Valid supplier is required.');
    }
    ...
    if (
        status !== undefined &&
        !['draft', 'ordered', 'received', 'cancelled'].includes(status)
    ) {
        errors.push('Invalid purchase order status.');
    }
    ...
    if (expectedDate && orderDate && expectedDate < orderDate) {
        errors.push('Expected date cannot be before order date.');
    }
